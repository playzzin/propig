...
exports.recommendMandalart = void 0;
...
const MandalartRecommendationSchema = zod_1.z.object({
    ...
    subGoals: zod_1.z
        ...
        tasks: zod_1.z.array(MandalartTaskSchema).length(8),
    }))
        .length(8),
});
const RecommendMandalartRequestSchema = zod_1.z.object({
    mainGoal: zod_1.z.string().min(1).max(50),
    template: zod_1.z.enum(['custom', 'okr', 'achievement']).optional(),
});
const getEnv = () => {
    var _a, _b;
    return {
        geminiApiKey: (_a = process.env.GEMINI_API_KEY) !== null && _a !== void 0 ? _a : '',
        geminiModel: (_b = process.env.GEMINI_MODEL) !== null && _b !== void 0 ? _b : 'gemini-2.5-flash',
    };
};
...
const buildSystemPrompt = (template) => {
    if (template === 'okr') {
...
exports.recommendMandalart = v2_1.https.onCall({ secrets: [secrets_1.geminiApiKey] }, async (request) => {
    ...
    const env = getEnv();
    if (!env.geminiApiKey) {
        throw new v2_1.https.HttpsError('failed-precondition', 'GEMINI_API_KEY 환경변수가 설정되지 않았습니다.');
    }
    const parsed = RecommendMandalartRequestSchema.safeParse(request.data);
    if (!parsed.success) {
        throw new v2_1.https.HttpsError('invalid-argument', '요청 형식이 올바르지 않습니다.', {
            issues: parsed.error.issues,
        });
    }
    const input = parsed.data;
    const template = (_a = input.template) !== null && _a !== void 0 ? _a : 'custom';
    try {
        const genAI = new generative_ai_1.GoogleGenerativeAI(env.geminiApiKey);
        ...
        const recommendation = MandalartRecommendationSchema.parse(json);
        v2_1.logger.info('Mandalart recommendation generated', {
        ...
        v2_1.logger.error('Failed to generate mandalart recommendation', error);
        const message = error instanceof Error ? error.message : 'Unknown error';
        throw new v2_1.https.HttpsError('internal', `만다라트 추천 생성에 실패했습니다: ${message}`);
    }
});
//# sourceMappingURL=recommendMandalart.js.map