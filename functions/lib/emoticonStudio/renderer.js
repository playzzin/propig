...
exports.resolveEmoticonOutputProfile = resolveEmoticonOutputProfile;
...
exports.inspectEmoticonAlphaIsolation = inspectEmoticonAlphaIsolation;
exports.evaluateEmoticonFrameVariation = evaluateEmoticonFrameVariation;
exports.resolveEmoticonBubbleFrame = resolveEmoticonBubbleFrame;
exports.extractVideoReviewFrame = extractVideoReviewFrame;
exports.inspectEmoticonRasterAlphaBounds = inspectEmoticonRasterAlphaBounds;
 ...
 */
function configureBundledKoreanFonts() {
    const functionsRoot = (0, node_path_1.resolve)(__dirname, '..', '..');
    ...
    ].map((segments) => (0, node_path_1.join)(functionsRoot, 'node_modules', ...segments));
    const missingDirectory = fontDirectories.find((directory) => !(0, node_fs_1.existsSync)(directory));
    if (missingDirectory) {
        throw new Error(`Bundled Korean font directory is missing: ${missingDirectory}`);
    }
    const configDirectory = (0, node_path_1.join)((0, node_os_1.tmpdir)(), 'propig-emoticon-fontconfig');
    const cacheDirectory = (0, node_path_1.join)(configDirectory, 'cache');
    const configPath = (0, node_path_1.join)(configDirectory, 'fonts.conf');
...
const SERVER_FORMAT_CAPABILITIES = {
    png: { alpha: 'supported' },
...
const VERIFIED_OUTPUT_PROFILES = {
...
function isAllowlistedVerifiedProfile(profile) {
...
function resolveEmoticonOutputProfile(profile) {
    if (!profile)
    ...
    if (!allowlisted) {
        return Object.assign(Object.assign({}, profile), { verification: 'reference', formatCapabilities: SERVER_FORMAT_CAPABILITIES, submissionCandidate: false });
    }
    ...
    const resolved = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ platform: allowlisted.platform, type: allowlisted.type, width: allowlisted.width, height: allowlisted.height, profileVersion: profile.profileVersion, verification: 'verified', transparentBackground: allowlisted.transparentBackground, sourceUrl: allowlisted.sourceUrl, checkedAt: allowlisted.checkedAt, allowedFormats: [...allowlisted.allowedFormats] }, (allowlisted.minFrameCount !== undefined
        ...
        : {})), { formatCapabilities: SERVER_FORMAT_CAPABILITIES, submissionCandidate: false });
    return resolved;
}
function assertOutputProfileConstraints(params) {
    if (params.outputProfile.verification !== 'verified')
        return;
    const effectiveFrameCount = params.outputProfile.type === 'static'
        ? 1
        : params.plan.action.frameCount;
    if (params.outputProfile.allowedFormats
    ...
    if (params.outputProfile.minFrameCount !== undefined
    ...
    if (params.outputProfile.maxFrameCount !== undefined
    ...
    if (params.outputProfile.maxDurationMs !== undefined
 ...
 * character outline.
 */
async function removeConnectedSmoothBackground(pngBuffer) {
    const { data, info } = await sharp(pngBuffer)
        .ensureAlpha()
        .raw()
        .toBuffer({ resolveWithObject: true });
    ...
    for (let pixelIndex = 0; pixelIndex < pixelCount; pixelIndex += 1) {
        if (data[pixelIndex * channels + alphaOffset] < 245)
            transparentPixels += 1;
    }
    if (transparentPixels / pixelCount >= 0.005)
        return pngBuffer;
    ...
    const enqueueNeighbour = (currentPixelIndex, candidatePixelIndex) => {
        if (visited[candidatePixelIndex])
            return;
        ...
        const distanceSquared = ((data[currentOffset] - data[candidateOffset]) ** 2
    ...
    while (queueStart < queueEnd) {
        ...
        const y = Math.floor(pixelIndex / width);
        if (x > 0)
            enqueueNeighbour(pixelIndex, pixelIndex - 1);
        if (x + 1 < width)
            enqueueNeighbour(pixelIndex, pixelIndex + 1);
        if (y > 0)
            enqueueNeighbour(pixelIndex, pixelIndex - width);
        if (y + 1 < height)
            enqueueNeighbour(pixelIndex, pixelIndex + width);
    }
    const removedRatio = queueEnd / pixelCount;
    if (removedRatio < 0.05 || removedRatio > 0.94)
        return pngBuffer;
    for (let index = 0; index < queueEnd; index += 1) {
 ...
 */
async function removeGroundShadowBands(pngBuffer) {
    const { data, info } = await sharp(pngBuffer)
        .ensureAlpha()
        .raw()
        .toBuffer({ resolveWithObject: true });
    ...
    for (let start = scanStartY * width; start < pixelCount; start += 1) {
        ...
        while (queueStart < queueEnd) {
            ...
            for (const neighbour of [
                ...
                if (neighbour < 0 || neighbour >= pixelCount)
                    continue;
                const neighbourY = Math.floor(neighbour / width);
                const neighbourX = neighbour - neighbourY * width;
                if (Math.abs(neighbourX - x) + Math.abs(neighbourY - y) !== 1)
                    continue;
                if (!candidate[neighbour] || labels[neighbour] !== -1)
                    continue;
 ...
 */
async function removeDistantResidualComponents(pngBuffer, options = {}) {
    const { data, info } = await sharp(pngBuffer)
        .ensureAlpha()
        .raw()
        .toBuffer({ resolveWithObject: true });
    ...
    const mainIndex = mainCandidates.reduce((bestIndex, index) => (components[index].count > components[bestIndex].count ? index : bestIndex), mainCandidates[0]);
    ...
    components.forEach((component, index) => {
        ...
        if (component.count >= minArtifactPixels)
            return;
...
async function sanitizeTransparentPixelRgb(pngBuffer) {
    const { data, info } = await sharp(pngBuffer)
        .ensureAlpha()
        .raw()
        .toBuffer({ resolveWithObject: true });
...
async function normalizeGeneratedEmoticonPose(buffer, options = {}) {
 ...
 */
async function inspectEmoticonAlphaIsolation(pngBuffer) {
    const { data, info } = await sharp(pngBuffer)
        .ensureAlpha()
        .raw()
        .toBuffer({ resolveWithObject: true });
...
async function createAlignedFrameVariationFingerprint(buffer) {
    const { data } = await sharp(buffer)
        .ensureAlpha()
...
function calculateFrameSilhouetteDifference(first, second) {
    if (first.length !== second.length || first.length % 4 !== 0)
        return 1;
    let difference = 0;
 ...
 */
async function evaluateEmoticonFrameVariation(frameBuffers, options = {}) {
    ...
    const alignedTransitionDifferences = alignedFingerprints.map((frame, index) => (calculateFrameDifference(frame, alignedFingerprints[(index + 1) % alignedFingerprints.length])));
    ...
    const allPairDuplicateFrameIndices = new Set();
    for (let firstIndex = 0; firstIndex < fingerprints.length - 1; firstIndex += 1) {
        for (let secondIndex = firstIndex + 1; secondIndex < fingerprints.length; secondIndex += 1) {
            const rgbaDifference = calculateFrameDifference(fingerprints[firstIndex], fingerprints[secondIndex]);
            const silhouetteDifference = calculateFrameSilhouetteDifference(fingerprints[firstIndex], fingerprints[secondIndex]);
            if (rgbaDifference < FRAME_NON_ADJACENT_DUPLICATE_THRESHOLD
    ...
    const meaningfulAlignedTransitions = chronologicalAlignedDifferences
        ...
        .length;
    const requiredMeaningfulTransitions = Math.max(2, Math.ceil(Math.max(1, frameBuffers.length - 1) * 0.4));
    const rigidMotionOnly = Boolean(options.requireSilhouetteVariation
...
async function runFfmpegCapture(args) {
 ...
 * the schema default.
 */
function getBubbleTimeline(bubble) {
    return bubble.timeline
        || LEGACY_BUBBLE_TIMELINE;
}
function assertBubbleTimeline(bubble, frameCount) {
...
function resolveEmoticonBubbleFrame(bubble, frameIndex, frameCount) {
    var _a, _b, _c;
    if (bubble.style === 'none' || frameIndex < 0 || frameIndex >= frameCount)
        return null;
    ...
    if (timeline.mode === 'cues' || !bubble.text.trim())
        return null;
    const lastFrame = frameCount - 1;
    let startFrame = 0;
    let endFrame = lastFrame;
    if (timeline.mode === 'intro') {
        endFrame = (_a = timeline.endFrame) !== null && _a !== void 0 ? _a : Math.max(0, Math.ceil(frameCount * 0.35) - 1);
    }
    else if (timeline.mode === 'outro') {
        startFrame = timeline.startFrame > 0
            ? timeline.startFrame
            : Math.min(lastFrame, Math.floor(frameCount * 0.65));
        endFrame = (_b = timeline.endFrame) !== null && _b !== void 0 ? _b : lastFrame;
    }
    else if (timeline.mode === 'range') {
        startFrame = timeline.startFrame;
        endFrame = (_c = timeline.endFrame) !== null && _c !== void 0 ? _c : lastFrame;
    }
    if (frameIndex < startFrame || frameIndex > endFrame)
        return null;
    return { text: bubble.text.trim(), startFrame, endFrame };
}
function bubbleOpacity(plan, frameIndex, resolved) {
    if (plan.bubble.entrance === 'none')
        return 1;
    const rangeLength = resolved.endFrame - resolved.startFrame + 1;
    const entranceFrames = Math.max(1, Math.min(rangeLength, Math.max(2, Math.round(plan.action.frameCount * 0.2))));
 ...
 */
function getBubbleAppearance(bubble) {
    const legacy = bubble;
    const safeColor = (value, fallback) => (typeof value === 'string' && BUBBLE_HEX_COLOR.test(value) ? value : fallback);
    return {
        size: clampNumber(legacy.size, 0.75, 1.2, schema_1.EMOTICON_BUBBLE_APPEARANCE_DEFAULTS.size),
        fillColor: safeColor(legacy.fillColor, schema_1.EMOTICON_BUBBLE_APPEARANCE_DEFAULTS.fillColor),
        textColor: safeColor(legacy.textColor, schema_1.EMOTICON_BUBBLE_APPEARANCE_DEFAULTS.textColor),
        outlineColor: safeColor(legacy.outlineColor, schema_1.EMOTICON_BUBBLE_APPEARANCE_DEFAULTS.outlineColor),
        outlineWidth: clampNumber(legacy.outlineWidth, 0, 10, schema_1.EMOTICON_BUBBLE_APPEARANCE_DEFAULTS.outlineWidth),
        shadowOpacity: clampNumber(legacy.shadowOpacity, 0, 0.75, schema_1.EMOTICON_BUBBLE_APPEARANCE_DEFAULTS.shadowOpacity),
        offsetX: Math.round(clampNumber(legacy.offsetX, -48, 48, schema_1.EMOTICON_BUBBLE_APPEARANCE_DEFAULTS.offsetX)),
        offsetY: Math.round(clampNumber(legacy.offsetY, -48, 48, schema_1.EMOTICON_BUBBLE_APPEARANCE_DEFAULTS.offsetY)),
    };
}
...
function getBubbleLayout(plan, resolved, position = plan.bubble.position) {
    if (!resolved)
        return null;
    ...
    const baseWidth = Math.min(isSide ? 190 : 314, Math.max(isSide ? 126 : 142, Math.max(...lines.map((line) => line.length)) * (isSide ? 18 : 24) + 42));
...
function getBubbleFontStyle(plan) {
...
function createBubbleSvg(plan, frameIndex, resolved, layout = getBubbleLayout(plan, resolved)) {
    if (!layout)
        return null;
    ...
    const radius = Math.round((plan.bubble.style === 'rounded' || plan.bubble.style === 'whisper' ? 24 : 16)
        * appearance.size);
    ...
    const text = lines.map((line, index) => (`<text x="${centerX}" y="${textY + index * lineGap}" text-anchor="middle" `
        ...
        + `fill="${appearance.textColor}">${escapeXml(line)}</text>`)).join('');
...
async function applyImageEditCropAndFlip(buffer, recipe) {
    if (!recipe)
        return buffer;
    const hasCrop = Object.values(recipe.crop).some((value) => value > 0);
    if (!hasCrop && !recipe.flipHorizontal)
        return buffer;
...
async function applyImageEditColorAdjustments(buffer, recipe) {
    var _a, _b, _c;
    const brightness = (_a = recipe === null || recipe === void 0 ? void 0 : recipe.brightness) !== null && _a !== void 0 ? _a : 0;
    const contrast = (_b = recipe === null || recipe === void 0 ? void 0 : recipe.contrast) !== null && _b !== void 0 ? _b : 0;
    const saturation = (_c = recipe === null || recipe === void 0 ? void 0 : recipe.saturation) !== null && _c !== void 0 ? _c : 0;
    ...
    if (contrast !== 0) {
        ...
        adjusted = await sharp(adjusted)
            .ensureAlpha()
            .linear([multiplier, multiplier, multiplier, 1], [offset, offset, offset, 0])
            .png()
            .toBuffer();
    }
    if (saturation !== 0) {
...
async function applyImageEditToCharacter(buffer, recipe) {
...
function applyImageEditToMotionTransform(transform, recipe) {
    if (!recipe)
        return transform;
    return {
...
function getEditedCharacterRegion(plan, recipe) {
    if (!recipe)
        return null;
...
function resolveBubblePlacement(params) {
    ...
    return Object.assign({ layout: requestedLayout }, requestedPlacement);
}
function getCharacterRegionBesideBubble(plan, layout) {
    if (!layout)
...
async function composeFrame(params) {
    ...
    const configuredBubbleLayers = ((_a = params.plan.bubbleLayers) === null || _a === void 0 ? void 0 : _a.length)
        ? params.plan.bubbleLayers
        : [params.plan.bubble];
    const resolvedBubbleLayers = configuredBubbleLayers.map((bubble) => {
        const plan = Object.assign(Object.assign({}, params.plan), { bubble });
        return {
            plan,
            resolved: resolveEmoticonBubbleFrame(bubble, params.frameIndex, params.plan.action.frameCount),
        };
    });
    ...
    if ((transformedMeta.width || 1) > availableRegion.width
    ...
    for (const layer of resolvedBubbleLayers) {
        ...
        if (bubbleOverlay)
            composites.push({ input: bubbleOverlay, left: 0, top: 0 });
    }
...
function validateFrameDurations(frameDurationsMs, frameCount) {
    if (!frameDurationsMs)
        return null;
    ...
    if (normalized.reduce((sum, duration) => sum + duration, 0) > 60000) {
        throw new Error('The imported animation duration must not exceed 60 seconds.');
    }
    return normalized;
}
async function stackAnimationFrames(frames) {
    var _a, _b;
    const metadata = await Promise.all(frames.map((frame) => sharp(frame).metadata()));
    const width = ((_a = metadata[0]) === null || _a === void 0 ? void 0 : _a.width) || 0;
    const height = ((_b = metadata[0]) === null || _b === void 0 ? void 0 : _b.height) || 0;
...
async function writeVariableFrameManifest(dir, frameDurationsMs, filePrefix = 'frame') {
    const manifestPath = (0, node_path_1.join)(dir, `${filePrefix}-timing.ffconcat`);
    ...
    await (0, promises_1.writeFile)(manifestPath, `${lines.join('\n')}\n`, 'utf8');
    return manifestPath;
}
async function extractVideoFrames(params) {
    ...
    return Promise.all(rawFrames.map(async (frame, frameIndex) => {
        ...
        const resolvedBubble = resolveEmoticonBubbleFrame(params.plan.bubble, frameIndex, params.plan.action.frameCount);
...
async function encodeApng(dir, fps, loopCount) {
    const outputPath = (0, node_path_1.join)(dir, 'emoticon.apng.png');
    await runFfmpeg([
        '-y',
        '-framerate',
        String(fps),
...
async function encodeVariableApng(dir, frameDurationsMs, loopCount) {
...
async function encodeVariableWebp(frames, frameDurationsMs) {
...
async function encodeVariableGif(frames, frameDurationsMs) {
    return (await stackAnimationFrames(frames))
        .gif({
        loop: 0,
        delay: frameDurationsMs,
...
async function encodeMp4(dir, fps, outputProfile) {
...
async function encodeVariableMp4(dir, frames, frameDurationsMs, outputProfile) {
    const opaqueDir = (0, node_path_1.join)(dir, 'opaque-video-frames');
    (0, node_fs_1.mkdirSync)(opaqueDir, { recursive: true });
    ...
    await Promise.all(opaqueFrames.map((frame, index) => ((0, promises_1.writeFile)((0, node_path_1.join)(opaqueDir, `opaque-${String(index + 1).padStart(3, '0')}.png`), frame))));
...
async function encodeVariableWebm(dir, frameDurationsMs) {
    const manifest = await writeVariableFrameManifest(dir, frameDurationsMs);
    const outputPath = (0, node_path_1.join)(dir, 'emoticon.webm');
    await runFfmpeg([
        '-y',
...
async function encodePngZip(frames, plan, outputProfile, frameDurationsMs) {
    const zip = new JSZip();
    ...
    const spriteManifest = {
        ...
        frames: frames.map((_, index) => {
            ...
            return ({
                ...
                width: outputProfile.width,
                height: outputProfile.height,
                durationMs: (_a = frameDurationsMs === null || frameDurationsMs === void 0 ? void 0 : frameDurationsMs[index]) !== null && _a !== void 0 ? _a : Math.round(1000 / plan.action.fps),
            });
        }),
    };
...
function requiredTransparentMarginPx(width, height) {
    return Math.max(MINIMUM_TRANSPARENT_MARGIN_PX, Math.ceil(Math.min(width, height) * MINIMUM_TRANSPARENT_MARGIN_RATIO));
}
function measureDecodedAlphaBounds(params) {
    ...
    const frames = Array.from({ length: params.frameCount }, (_, frameIndex) => {
        ...
        for (let y = 0; y < params.frameHeight; y += 1) {
            for (let x = 0; x < params.width; x += 1) {
                ...
                if (((_a = params.data[offset]) !== null && _a !== void 0 ? _a : 0) < ALPHA_BOUNDS_THRESHOLD)
                    continue;
                visiblePixelCount += 1;
        ...
        const minimumTransparentMarginPx = Math.min(transparentMargins.left, transparentMargins.top, transparentMargins.right, transparentMargins.bottom);
    ...
    return {
        ...
        allFramesHaveUsableTransparentMargin: frames.every((frame) => frame.hasUsableTransparentMargin),
...
async function inspectRasterBuffer(buffer, animated, fallbackFps) {
    const options = animated ? { animated: true } : {};
...
function parseApngControl(buffer) {
    ...
    while (offset + 12 <= buffer.length) {
        ...
        if (dataEnd + 4 > buffer.length)
            throw new Error('APNG output contains a truncated PNG chunk.');
    ...
    if (!frameCount)
        throw new Error('Encoded PNG does not contain an APNG animation control chunk.');
    return { frameCount, plays, durationMs };
}
async function inspectApngBuffer(params) {
    const control = parseApngControl(params.buffer);
    const inputPath = (0, node_path_1.join)(params.dir, 'inspection.apng.png');
    const decodedDir = (0, node_path_1.join)(params.dir, 'inspection-apng-frames');
    (0, node_fs_1.mkdirSync)(decodedDir, { recursive: true });
    await (0, promises_1.writeFile)(inputPath, params.buffer);
    await runFfmpeg([
        '-y',
        '-i',
        inputPath,
        ...
        '-fps_mode',
        'passthrough',
        (0, node_path_1.join)(decodedDir, 'decoded-%03d.png'),
    ]);
    const decodedNames = (await (0, promises_1.readdir)(decodedDir))
        .filter((name) => /^decoded-\d{3}\.png$/.test(name))
        .sort();
    const decodedFrames = await Promise.all(decodedNames.map((name) => (0, promises_1.readFile)((0, node_path_1.join)(decodedDir, name))));
    ...
    const frameFacts = await Promise.all(decodedFrames.map((frame) => inspectRasterBuffer(frame, false, params.fallbackFps)));
    ...
    const alphaFrames = frameFacts.map((frame, frameIndex) => {
        var _a;
        const evidence = (_a = frame.alphaBoundsEvidence) === null || _a === void 0 ? void 0 : _a.frames[0];
        if (!evidence)
            throw new Error(`Decoded APNG frame ${frameIndex + 1} has no alpha-bound evidence.`);
        return Object.assign(Object.assign({}, evidence), { frameIndex });
    });
    const alphaBoundsEvidence = {
        alphaThreshold: ALPHA_BOUNDS_THRESHOLD,
        requiredTransparentMarginPx: Math.max(...frameFacts.map((frame) => { var _a; return ((_a = frame.alphaBoundsEvidence) === null || _a === void 0 ? void 0 : _a.requiredTransparentMarginPx) || 1; })),
    ...
    return {
        ...
        transparencyCoverage: frameFacts.reduce((sum, frame) => sum + (frame.transparencyCoverage || 0), 0) / frameFacts.length,
        alphaBoundsEvidence,
        codec: 'apng',
    };
}
async function inspectEmoticonRasterAlphaBounds(buffer, animated = false) {
...
function getLastFfmpegTimeMs(stderr) {
    const expression = /time=\s*(\d+:\d+:\d+(?:\.\d+)?)/g;
    let match;
    let result = null;
    while ((match = expression.exec(stderr)) !== null)
        result = parseFfmpegTimeMs(match[1]);
    return result;
}
function getFfmpegDeclaredDurationMs(stderr) {
...
async function inspectVideoBuffer(params) {
    var _a, _b, _c;
    const inputPath = (0, node_path_1.join)(params.dir, `inspection.${params.extension}`);
    await (0, promises_1.writeFile)(inputPath, params.buffer);
    ...
    const codec = ((_a = videoLine.match(/Video:\s*([^,\s]+)/)) === null || _a === void 0 ? void 0 : _a[1]) || null;
    const fpsMatch = (_b = videoLine.match(/(\d+(?:\.\d+)?)\s+fps\b/i)) === null || _b === void 0 ? void 0 : _b[1];
    ...
    const durationMs = (_c = getFfmpegDeclaredDurationMs(stderr)) !== null && _c !== void 0 ? _c : getLastFfmpegTimeMs(stderr);
    const inspectionIssues = [];
    if (!dimension)
        inspectionIssues.push('Encoded video dimensions could not be decoded.');
    if (!frameCount)
        inspectionIssues.push('Encoded video frame count could not be decoded.');
    if (durationMs === null)
        inspectionIssues.push('Encoded video duration could not be decoded.');
...
function buildOutputInspection(params) {
    ...
    if (params.facts.width !== params.outputProfile.width || params.facts.height !== params.outputProfile.height) {
        issues.push(`Actual canvas ${params.facts.width || '?'}x${params.facts.height || '?'} does not match `
    ...
    if (params.format !== 'png' && params.facts.fps === null) {
        ...
        issues.push(`Actual FPS ${params.facts.fps} does not match expected ${expectedFps}.`);
    }
    if (params.format !== 'png'
        && params.facts.durationMs !== null
        && Math.abs(params.facts.durationMs - expectedDurationMs) > Math.max(120, params.expectedFrameDurationsMs
            ? Math.max(...params.expectedFrameDurationsMs)
            : Math.ceil(1000 / params.plan.action.fps))) {
        issues.push(`Actual duration ${params.facts.durationMs}ms does not match expected `
            + `${expectedDurationMs}ms.`);
    }
    const alphaCapability = (_b = (_a = params.outputProfile.formatCapabilities) === null || _a === void 0 ? void 0 : _a[params.format]) === null || _b === void 0 ? void 0 : _b.alpha;
    ...
    if (requiresAlphaBoundsEvidence) {
        ...
        if (!evidence || evidence.checkedFrameCount !== expectedFrameCount) {
            ...
            if (evidence.edgeTouchFrameIndices.length > 0) {
                issues.push(`Opaque content touches a canvas edge in frame(s): ${evidence.edgeTouchFrameIndices.join(', ')}.`);
            }
            if (!evidence.allFramesHaveUsableTransparentMargin) {
                issues.push(`Transparent margin is smaller than ${evidence.requiredTransparentMarginPx}px in frame(s): `
                    + `${evidence.insufficientMarginFrameIndices.join(', ')}.`);
            }
        }
    }
    if (params.outputProfile.type === 'animated'
        && (params.format === 'webp'
            || params.format === 'gif'
...
async function inspectPngZip(params) {
    ...
    const frameBuffers = await Promise.all(frameNames.map(async (name) => {
        ...
        if (!entry)
            throw new Error(`PNG sequence entry ${name} is missing.`);
        return entry.async('nodebuffer');
    }));
    if (!frameBuffers.length)
        throw new Error('PNG sequence archive contains no frame images.');
    ...
    if (facts.some((frame) => (frame.transparencyCoverage !== null
        && 1 - frame.transparencyCoverage < 0.005))) {
        issues.push('At least one PNG sequence frame is empty or almost completely transparent.');
    }
    if (params.outputProfile.transparentBackground && facts.some((frame) => (frame.transparencyCoverage !== null
        && frame.transparencyCoverage < 0.005))) {
        issues.push('At least one PNG sequence frame has no usable transparent margin.');
    }
    const averageTransparency = facts.reduce((sum, frame) => sum + (frame.transparencyCoverage || 0), 0) / facts.length;
    const alphaBoundsFrames = facts.map((frame, frameIndex) => {
        var _a;
        const frameEvidence = (_a = frame.alphaBoundsEvidence) === null || _a === void 0 ? void 0 : _a.frames[0];
        if (!frameEvidence) {
            throw new Error(`PNG sequence frame ${frameIndex + 1} has no alpha-bound evidence.`);
        }
        return Object.assign(Object.assign({}, frameEvidence), { frameIndex });
    });
    const alphaBoundsEvidence = {
        alphaThreshold: ALPHA_BOUNDS_THRESHOLD,
        requiredTransparentMarginPx: Math.max(...facts.map((frame) => {
            var _a;
            return (((_a = frame.alphaBoundsEvidence) === null || _a === void 0 ? void 0 : _a.requiredTransparentMarginPx)
        ...
        allFramesHaveUsableTransparentMargin: alphaBoundsFrames.every((frame) => frame.hasUsableTransparentMargin),
...
async function encodeRequestedFormats(dir, frames, fps, formats, plan, outputProfile, requestedFrameDurationsMs) {
    var _a;
    ...
    for (const format of formats) {
        ...
        if (format === 'png') {
            ...
            inspection = buildOutputInspection(Object.assign(Object.assign({ format,
                buffer,
                plan,
                outputProfile }, (frameDurationsMs ? { expectedFrameDurationsMs: frameDurationsMs } : {})), { facts: await inspectRasterBuffer(buffer, false, fps) }));
        }
        else if (format === 'apng') {
            extension = 'apng.png';
            contentType = 'image/png';
            const loopCount = (_a = outputProfile.loopCount) !== null && _a !== void 0 ? _a : 0;
            ...
            inspection = buildOutputInspection(Object.assign(Object.assign({ format,
                buffer,
                plan,
                outputProfile }, (frameDurationsMs ? { expectedFrameDurationsMs: frameDurationsMs } : {})), { facts: await inspectApngBuffer({ dir, buffer, fallbackFps: fps }) }));
        }
        else if (format === 'webp') {
            extension = 'webp';
            ...
            inspection = buildOutputInspection(Object.assign(Object.assign({ format,
                buffer,
                plan,
                outputProfile }, (frameDurationsMs ? { expectedFrameDurationsMs: frameDurationsMs } : {})), { facts: await inspectRasterBuffer(buffer, true, fps) }));
        }
        else if (format === 'gif') {
            extension = 'gif';
            ...
            inspection = buildOutputInspection(Object.assign(Object.assign({ format,
                buffer,
                plan,
                outputProfile }, (frameDurationsMs ? { expectedFrameDurationsMs: frameDurationsMs } : {})), { facts: videoFacts, extraIssues: videoFacts.inspectionIssues }));
        }
        else {
            extension = 'zip';
            contentType = 'application/zip';
            buffer = await encodePngZip(frames, plan, outputProfile, frameDurationsMs || undefined);
            inspection = await inspectPngZip(Object.assign({ buffer,
                plan,
                outputProfile }, (frameDurationsMs ? { frameDurationsMs } : {})));
        }
...
async function renderImageEmoticon(params) {
    return withTempDir('emoticon-image-', async (dir) => {
        ...
        const character = await applyImageEditToCharacter(await normalizeCharacter(params.imageBuffer, params.plan), params.editRecipe);
        const frames = [];
        ...
        const [files] = await Promise.all([
            encodeRequestedFormats(dir, frames, params.plan.action.fps, params.formats, params.plan, outputProfile),
 ...
 */
async function applyFrameTransitions(frames, transitions) {
    if (frames.length < 2 || transitions.length !== frames.length)
        return frames;
    ...
    if (channels !== 4 || decoded.some((entry) => (entry.info.width !== width || entry.info.height !== height || entry.info.channels !== 4)))
        return frames;
    return Promise.all(decoded.map(async (current, index) => {
        ...
        const copyPixel = (source, sourceX, sourceY, targetOffset) => {
            const sourceOffset = (Math.min(height - 1, Math.max(0, sourceY)) * width
                + Math.min(width - 1, Math.max(0, sourceX))) * 4;
            for (let channel = 0; channel < 4; channel += 1)
                output[targetOffset + channel] = source[sourceOffset + channel];
        };
        for (let y = 0; y < height; y += 1) {
            for (let x = 0; x < width; x += 1) {
                ...
                if (transition.kind === 'fade') {
                    ...
                    if (x < width - shift)
                        copyPixel(previous.data, x + shift, y, offset);
                    else
                        copyPixel(current.data, x - (width - shift), y, offset);
                }
                    ...
                    if (x >= shift)
                        ...
                        copyPixel(current.data, width - shift + x, y, offset);
                }
                else {
                    ...
                    if (x >= left && x < left + scaledWidth && y >= top && y < top + scaledHeight) {
                        ...
                        copyPixel(previous.data, x, y, offset);
                }
            }
        }
...
async function renderImageSequenceEmoticon(params) {
    ...
    return withTempDir('emoticon-image-sequence-', async (dir) => {
        ...
        const characters = await Promise.all(normalizedCharacters.map((character) => (applyImageEditToCharacter(character, params.editRecipe))));
        ...
        const frames = ((_a = params.frameTransitions) === null || _a === void 0 ? void 0 : _a.length) === composedFrames.length
        ...
        const composedFrameCheckpoint = params.onComposedFrames
            ...
            : Promise.resolve();
        const [files] = await Promise.all([
            encodeRequestedFormats(dir, frames, params.plan.action.fps, params.formats, params.plan, outputProfile, params.frameDurationsMs),
            composedFrameCheckpoint,
        ]);
        return files;
    });
...
async function renderVideoEmoticon(params) {
    return withTempDir('emoticon-video-', async (dir) => {
        ...
        return encodeRequestedFormats(dir, profiledFrames, params.plan.action.fps, params.formats, params.plan, outputProfile);
    });
}
