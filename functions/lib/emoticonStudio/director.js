...
const cachedPresetsSchema = schema_1.emoticonPresetSchema.array().min(4).max(12);
...
async function requestStructuredChat(params) {
    ...
    const requestBody = Object.assign(Object.assign({ model: params.model }, (params.fallbackModels.length
        ? { models: params.fallbackModels.filter((model) => model !== params.model) }
        : {})), { messages: params.messages, temperature: 0.25, max_tokens: params.maxTokens, response_format: {
            type: 'json_schema',
            json_schema: {
    ...
    try {
        ...
        if (controller.signal.aborted)
            throw new Error('OpenRouter structured analysis timed out.');
        throw error;
    }
    finally {
    ...
    if (!response.ok) {
        throw new Error(((_a = payload.error) === null || _a === void 0 ? void 0 : _a.message) || raw.slice(0, 1000) || `OpenRouter HTTP ${response.status}`);
    }
...
function normalizeEmoticonPlanTiming(plan) {
    const fps = Math.max(6, Math.min(18, Math.round(plan.action.fps)));
    let frameCount = Math.max(8, Math.min(24, Math.round((plan.action.durationMs / 1000) * fps)));
    if ((frameCount / fps) > 3)
        frameCount = Math.max(8, Math.floor(3 * fps));
    if ((frameCount / fps) < 0.7)
        frameCount = Math.min(24, Math.ceil(0.7 * fps));
    const durationMs = Math.max(700, Math.min(3000, Math.round((frameCount / fps) * 1000)));
    return Object.assign(Object.assign({}, plan), { action: Object.assign(Object.assign({}, plan.action), { fps,
            frameCount,
            durationMs }) });
}
/**
...
function applyEmoticonMotionPolicy(params) {
    ...
    return Object.assign(Object.assign({}, plan), { action: Object.assign(Object.assign({}, plan.action), { renderMode: 'dynamic', motionType: 'dynamic', fps: 8, durationMs: 1000 }) });
}
    ...
    if (useCachedAnalysis) {
        ...
        if (!parsed.success) {
            throw new Error(`OpenRouter returned an invalid cached emoticon plan: `
                + `${((_d = parsed.error.issues[0]) === null || _d === void 0 ? void 0 : _d.message) || 'unknown shape'}`);
        }
        ...
        if (!parsed.success) {
            throw new Error(`OpenRouter returned an invalid emoticon plan: `
                + `${((_e = parsed.error.issues[0]) === null || _e === void 0 ? void 0 : _e.message) || 'unknown shape'}`);
        }
        plan = parsed.data;
    }
...
async function planEmoticonFrameSequence(params) {
    ...
    const content = ((_c = (_b = (_a = payload.choices) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.message) === null || _c === void 0 ? void 0 : _c.content) || '';
    const parsed = schema_1.emoticonFrameSequenceSchema.safeParse(extractJsonObject(content));
    if (!parsed.success) {
        throw new Error(`OpenRouter returned an invalid frame sequence: `
            + `${((_d = parsed.error.issues[0]) === null || _d === void 0 ? void 0 : _d.message) || 'unknown shape'}`);
    }
