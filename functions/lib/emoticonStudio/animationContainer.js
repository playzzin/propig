...
exports.EMOTICON_SPRITE_SHEET_STRATEGY = exports.MAX_EMOTICON_CONTAINER_PIXELS = exports.MAX_EMOTICON_CONTAINER_FRAMES = void 0;
exports.resolveEmoticonSpriteSheetLayout = resolveEmoticonSpriteSheetLayout;
exports.extractEmoticonSpriteSheetFrames = extractEmoticonSpriteSheetFrames;
exports.extractAnimationContainerFrames = extractAnimationContainerFrames;
// eslint-disable-next-line @typescript-eslint/no-require-imports
const sharp = require('sharp');
exports.MAX_EMOTICON_CONTAINER_FRAMES = 24;
exports.MAX_EMOTICON_CONTAINER_PIXELS = 96 * 1024 * 1024;
exports.EMOTICON_SPRITE_SHEET_STRATEGY = 'sprite-sheet-v1';
const AUTO_SPRITE_LAYOUTS = [
    { columns: 2, rows: 1 },
    { columns: 3, rows: 1 },
    { columns: 2, rows: 2 },
    { columns: 3, rows: 2 },
...
function resolveEmoticonSpriteSheetLayout(frameCount) {
    const normalized = Math.max(2, Math.min(exports.MAX_EMOTICON_CONTAINER_FRAMES, Math.round(frameCount)));
    if (normalized <= 2)
        return { columns: 2, rows: 1, frameCount: normalized };
    if (normalized <= 3)
        return { columns: 3, rows: 1, frameCount: normalized };
    if (normalized <= 4)
        return { columns: 2, rows: 2, frameCount: normalized };
    if (normalized <= 6)
        return { columns: 3, rows: 2, frameCount: normalized };
    if (normalized <= 8)
        return { columns: 4, rows: 2, frameCount: normalized };
    if (normalized <= 9)
        return { columns: 3, rows: 3, frameCount: normalized };
    if (normalized <= 12)
        return { columns: 4, rows: 3, frameCount: normalized };
    if (normalized <= 16)
        return { columns: 4, rows: 4, frameCount: normalized };
    if (normalized <= 20)
        return { columns: 5, rows: 4, frameCount: normalized };
    return { columns: 6, rows: 4, frameCount: normalized };
}
function pixelOffset(decoded, x, y) {
    return ((y * decoded.width) + x) * decoded.channels;
}
function colorDistanceSquared(decoded, offset, target) {
    return ((decoded.data[offset] - target[0]) ** 2
        + (decoded.data[offset + 1] - target[1]) ** 2
        + (decoded.data[offset + 2] - target[2]) ** 2);
}
function isSpriteBackgroundPixel(decoded, x, y) {
...
async function decodeSpriteSheet(source) {
    const { data, info } = await sharp(source, {
        failOn: 'error',
        limitInputPixels: exports.MAX_EMOTICON_CONTAINER_PIXELS,
    ...
    if (width < 192
        ...
        || width > 4096
        || height > 4096
        || width * height > exports.MAX_EMOTICON_CONTAINER_PIXELS) {
    ...
    const backgroundRgb = [0, 1, 2].map((channel) => Math.round(cornerOffsets.reduce((sum, offset) => sum + data[offset + channel], 0) / cornerOffsets.length));
    if (!transparentBackground) {
        const cornersAgree = cornerOffsets.every((offset) => colorDistanceSquared({ data, width, height, channels, transparentBackground, backgroundRgb }, offset, backgroundRgb) <= 72 ** 2);
        if (!cornersAgree) {
...
function inspectSpriteLayout(decoded, layout, options = {}) {
    var _a;
    const cellWidth = decoded.width / layout.columns;
    const cellHeight = decoded.height / layout.rows;
    if (cellWidth < 96 || cellHeight < 96 || layout.columns * layout.rows < layout.frameCount)
        return null;
    const ratioError = Math.abs(Math.log((decoded.width / decoded.height) / (layout.columns / layout.rows)));
    ...
    if (((_a = options.enforceCanvasRatio) !== null && _a !== void 0 ? _a : true) && ratioError > 0.16)
        return null;
    const borderRatios = [];
    const contentRatios = [];
    for (let index = 0; index < layout.frameCount; index += 1) {
        ...
        for (let step = 0; step < 24; step += 1) {
            ...
            for (const [sampleX, sampleY] of [
                ...
                borderSamples += 1;
                if (isSpriteBackgroundPixel(decoded, sampleX, sampleY))
                    borderBackground += 1;
            }
        }
        ...
        for (let sampleY = 1; sampleY <= 14; sampleY += 1) {
            for (let sampleX = 1; sampleX <= 14; sampleX += 1) {
                ...
                interiorSamples += 1;
                if (!isSpriteBackgroundPixel(decoded, x, y))
                    visibleSamples += 1;
            }
        }
        const contentRatio = visibleSamples / Math.max(1, interiorSamples);
        if (contentRatio < 0.025 || contentRatio > 0.82)
            return null;
    ...
    return {
        ...
        score: averageCellBorderBackgroundRatio - ratioError - Math.abs(0.34 - (contentRatios.reduce((sum, value) => sum + value, 0) / contentRatios.length)) * 0.2,
    };
}
function trailingSpriteCellsAreEmpty(decoded, layout) {
    const capacity = layout.columns * layout.rows;
    if (layout.frameCount >= capacity)
        return true;
    for (let index = layout.frameCount; index < capacity; index += 1) {
        ...
        if (nonBackgroundSamples / sampleCount > 0.012)
            return false;
    }
    return true;
}
function inferSpriteSheetLayout(decoded) {
    const candidates = AUTO_SPRITE_LAYOUTS.flatMap(({ columns, rows }) => {
        ...
        return Array.from({ length: capacity - 1 }, (_, offset) => offset + 2).flatMap((frameCount) => {
            ...
            if (!evidence || !trailingSpriteCellsAreEmpty(decoded, layout))
                return [];
            ...
            return [{
                    layout,
                    evidence: Object.assign(Object.assign({}, evidence), { score: evidence.score - unusedCellPenalty }),
                }];
        });
...
async function extractEmoticonSpriteSheetFrames(source, requestedLayout) {
    ...
    if (requestedLayout) {
        selected.layout = {
            ...
            frameCount: Math.max(2, Math.min(exports.MAX_EMOTICON_CONTAINER_FRAMES, Math.round(requestedLayout.frameCount))),
        };
...
function createSpriteBackgroundCandidate(rgba, offset, backgroundRgb, greenScreen) {
    const red = rgba[offset];
    const green = rgba[offset + 1];
    const blue = rgba[offset + 2];
    const distance = Math.sqrt((red - backgroundRgb[0]) ** 2
 ...
 */
async function extractAndMatteSpriteCell(decoded, region) {
    const channels = 4;
    ...
    for (let index = 0; index < pixelCount; index += 1) {
        const result = createSpriteBackgroundCandidate(rgba, index * channels, decoded.backgroundRgb, greenScreen);
    ...
    for (let index = 0; index < pixelCount; index += 1) {
        ...
        if (isMattePixel) {
            ...
            if (greenScreen && rgba[offset + 3] > 0 && rgba[offset + 3] < 250) {
                rgba[offset + 1] = Math.min(rgba[offset + 1], Math.max(rgba[offset], rgba[offset + 2]) + 8);
            }
        }
        if (rgba[offset + 3] <= 8) {
            rgba[offset] = 255;
...
function encodeSpriteCellWithSafeGutter(rgba, width, height) {
...
async function extractAnimationContainerFrames(source) {
    ...
    if (frameCount < 2
        || frameCount > exports.MAX_EMOTICON_CONTAINER_FRAMES
        || !width
        || !frameHeight
        || width > 4096
        || frameHeight > 4096
        || totalPixels > exports.MAX_EMOTICON_CONTAINER_PIXELS) {
