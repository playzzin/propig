...
import { getEmoticonStudioCostLimits } from './emoticonStudio/rateLimits';

export type OpenRouterUsageOperation = 'text' | 'image' | 'video';

export type OpenRouterUsageContext = {
    userId?: string;
    projectId?: string;
    jobId?: string;
    batchId?: string;
    ...
    seed?: number;
    stage?: string;
    /** Stable OpenRouter endpoint identity used for accounting and diagnostics. */
    providerSlug?: string;
    /** Public routing tag accepted by provider.only. Never substitute providerSlug. */
    providerTag?: string;
};
...
export type OpenRouterUsageReservation = {
    documentId: string;
    ...
    userId: string;
    jobId: string;
    dailyCostAggregateId: string;
    dailyCostDay: string;
};

export class OpenRouterDuplicateOperationError extends Error {
    readonly reservationState: 'inflight' | 'uncertain' | 'settled';

    constructor(state: 'inflight' | 'uncertain' | 'settled') {
...
export class OpenRouterDailyCostLimitError extends Error {
    readonly day: string;
    ...
    constructor(params: {
        ...
        super(
            `OpenRouter daily cost limit reached for ${params.day} (`
...
const usageContextStorage = new AsyncLocalStorage<OpenRouterUsageContext>();
...
export const OPENROUTER_LOGICAL_OPERATION_KEY_VERSION = 2;
export const OPENROUTER_DAILY_COST_AGGREGATE_VERSION = 1;
export const OPENROUTER_DAILY_COST_COLLECTION = 'openrouter_daily_costs';

export function runWithOpenRouterUsageContext<T>(
    context: OpenRouterUsageContext,
    task: () => Promise<T>,
): Promise<T> {
    return usageContextStorage.run(
        { ...(usageContextStorage.getStore() || {}), ...context },
        task,
    );
}

export function updateOpenRouterUsageContext(context: OpenRouterUsageContext): void {
...
const storageIdentifierFromUrl = (url: URL): string | null => {
...
const contentIdentifierFromDataUrl = (value: string): string | null => {
 ...
 */
export function createOpenRouterStableImageIdentifier(value: string): string {
    ...
    try {
        ...
        if (storageIdentifier) return storageIdentifier;
        url.hash = '';
        for (const key of [...url.searchParams.keys()]) {
            if (VOLATILE_URL_PARAMETER.test(key)) url.searchParams.delete(key);
        }
...
type StableRequestValue = null | boolean | number | string | StableRequestValue[] | {
    [key: string]: StableRequestValue;
};

const stableRequestValue = (value: unknown, ancestors = new Set<object>()): StableRequestValue => {
    ...
    if (value instanceof Uint8Array) {
        return `content-sha256:${createHash('sha256').update(value).digest('hex')}`;
    }
    if (value instanceof Date) return value.toISOString();
    if (Array.isArray(value)) {
        if (ancestors.has(value)) throw new Error('OpenRouter logical operation request cannot be cyclic.');
 ...
 */
export function createOpenRouterLogicalOperationId(params: {
    operation: OpenRouterUsageOperation;
    model: string;
    request: unknown;
}): string {
    ...
    return hash(JSON.stringify({
        keyVersion: OPENROUTER_LOGICAL_OPERATION_KEY_VERSION,
        ...
        jobId: typeof context.jobId === 'string' && SAFE_CONTEXT_ID.test(context.jobId)
            ...
            : null,
        stage: stableStage,
        frameIndex: Number.isInteger(context.frameIndex) && (context.frameIndex as number) >= 0
            ? context.frameIndex
            : null,
        seed: Number.isInteger(context.seed) && (context.seed as number) >= 0
            ? context.seed
            : null,
...
export const toKoreaDateKey = (date: Date): string => {
...
export const nextKoreaDayStartMs = (timestampMs: number): number => {
...
export type OpenRouterDailyCostAggregateIdentity = {
    documentId: string;
    day: string;
    userIdHash: string;
};

export function buildOpenRouterDailyCostAggregateIdentity(
    userIdHash: string,
    day: string,
): OpenRouterDailyCostAggregateIdentity | null {
    if (!SHA256_HEX.test(userIdHash) || !KOREA_DATE_KEY.test(day)) return null;
    return {
...
export function resolveOpenRouterDailyCostAggregateIdentity(
    ...
    if (
        typeof usage.dailyCostAggregateId === 'string'
        && usage.dailyCostAggregateId !== identity.documentId
    ) return null;
    return identity;
}

const authorizedJobCost = (job: Record<string, unknown>): number => {
...
export function canReserveOpenRouterCost(params: {
...
export function canReserveOpenRouterDailyCost(params: {
    dailyCostLimitUsd: number;
    actualCostUsd: number;
    inflightCostUsd: number;
    estimatedCostUsd: number;
...
export type OpenRouterDailyCostAggregateTransition = {
    actualCostUsd: number;
    inflightCostUsd: number;
};
 ...
 */
export function transitionOpenRouterDailyCostAggregate(params: {
    aggregate?: Record<string, unknown> | null;
    actualDeltaUsd?: number;
    inflightDeltaUsd?: number;
}): OpenRouterDailyCostAggregateTransition {
    const currentActual = safeStoredMoney(params.aggregate?.actualCostUsd);
    const currentInflight = safeStoredMoney(params.aggregate?.inflightCostUsd);
...
export function resolveOpenRouterDailyCostLimitUsd(params: {
    job?: Record<string, unknown> | null;
    usage?: Record<string, unknown> | null;
    aggregate?: Record<string, unknown> | null;
    environment?: Record<string, string | undefined>;
} = {}): number {
    const configured = getEmoticonStudioCostLimits(params.environment).dailyUsd;
    const storedLimits = [
        ['job.openRouterDailyCostLimitUsd', params.job?.openRouterDailyCostLimitUsd],
        ['usage.dailyCostLimitUsd', params.usage?.dailyCostLimitUsd],
        ['aggregate.dailyCostLimitUsd', params.aggregate?.dailyCostLimitUsd],
    ].flatMap(([field, value]) => {
...
function requireDailyAggregateMoney(
    aggregate: Record<string, unknown>,
    field: 'actualCostUsd' | 'inflightCostUsd',
): number {
...
export type OpenRouterUsageReconciliationDecision = 'charged' | 'not_charged';

export type OpenRouterUsageReconciliationErrorCode =
    | 'invalid_input'
    | 'invalid_data'
    | 'conflict';

export class OpenRouterUsageReconciliationError extends Error {
    readonly code: OpenRouterUsageReconciliationErrorCode;

    constructor(code: OpenRouterUsageReconciliationErrorCode, message: string) {
        super(message);
...
export type OpenRouterUsageReconciliationTransition = {
    outcome: 'updated' | 'idempotent';
    decision: OpenRouterUsageReconciliationDecision;
    accountedCostUsd: number;
    reservedCostUsd: number;
    reconciliationJobUpdated: boolean;
    usageUpdate: null | {
        state: 'settled' | 'failed';
        reservedCostUsd: 0;
        costUsd: number;
        costSource: 'admin_reconciliation';
        reconciliationDecision: OpenRouterUsageReconciliationDecision;
        reconciledActualCostUsd: number;
        reconciledByAdminUidHash: string;
        reconciliationJobUpdated: boolean;
        reconciliationNote?: string;
        failureReason?: 'admin_confirmed_not_charged';
    };
    jobUpdate: null | {
        openRouterInflightCostUsd: number;
        openRouterActualCostUsd: number;
        openRouterUsageRequestCount: number;
    };
};

const reconciliationError = (
    code: OpenRouterUsageReconciliationErrorCode,
    message: string,
): never => {
    throw new OpenRouterUsageReconciliationError(code, message);
};

const requireStoredCost = (
    value: unknown,
    field: string,
...
const optionalStoredCost = (value: unknown): number => (
    typeof value === 'number' && Number.isFinite(value) && value >= 0 && value <= 100
        ? value
        : 0
);

const safeStoredMoney = (value: unknown): number => (
    typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : 0
);

const safeStoredCounter = (value: unknown): number => (
 ...
 */
export function reconcileOpenRouterUsageState(input: {
    operationId: string;
    decision: OpenRouterUsageReconciliationDecision;
    actualCostUsd?: number;
    note?: string;
    ...
    usage: Record<string, unknown>;
    job?: Record<string, unknown> | null;
}): OpenRouterUsageReconciliationTransition {
    ...
    if (
        input.usage.provider !== 'openrouter'
    ...
    if (
        input.usage.reconciliationDecision === input.decision
    ...
    if (input.usage.state !== 'uncertain') {
        return reconciliationError(
            'conflict',
            'The usage operation is not awaiting reconciliation or has a different decision.',
        );
    }

    const reservedCostUsd = requireStoredCost(input.usage.reservedCostUsd, 'reservedCostUsd');
    const accountedCostUsd = input.decision === 'charged' ? input.actualCostUsd as number : 0;
    ...
    const usageJobId = typeof input.usage.jobId === 'string'
        ...
        ? input.usage.jobId
        : null;
    const jobUserId = typeof input.job?.userId === 'string' && input.job.userId.trim()
        ? input.job.userId.trim()
        : null;
    const jobId = typeof input.job?.id === 'string' ? input.job.id : null;
    const ownsJob = Boolean(
        input.job
        && usageUserIdHash
        && usageJobId
        && jobUserId
    ...
    let jobUpdate: OpenRouterUsageReconciliationTransition['jobUpdate'] = null;
    ...
    return {
        ...
        usageUpdate: {
            ...
            reconciledByAdminUidHash: input.reconciledByAdminUidHash,
            reconciliationJobUpdated,
            ...(note ? { reconciliationNote: note } : {}),
            ...(input.decision === 'not_charged'
                ? { failureReason: 'admin_confirmed_not_charged' as const }
                : {}),
        },
        jobUpdate,
    };
...
export async function assertOpenRouterUsageBudget(estimatedRequestCostUsd: number): Promise<void> {
    const { userId, jobId } = safeContext();
    if (!userId || !jobId) return;
    const jobSnapshot = await db.doc(`users/${userId}/emoticonJobs/${jobId}`).get();
    if (!jobSnapshot.exists) throw new Error('The OpenRouter cost budget job no longer exists.');
    const job = jobSnapshot.data() || {};
    const authorizedCostUsd = authorizedJobCost(job);
    const actualCostUsd = toNonNegativeNumber(job.openRouterActualCostUsd as number | undefined);
    const inflightCostUsd = toNonNegativeNumber(job.openRouterInflightCostUsd as number | undefined);
    ...
    if (!canReserveOpenRouterCost({
        ...
        throw new Error(
            `OpenRouter job cost budget reached (${actualCostUsd.toFixed(4)} actual + `
 ...
 */
export async function reserveOpenRouterUsage(params: {
    operation: OpenRouterUsageOperation;
    model: string;
    logicalOperationId: string;
    estimatedCostUsd: number;
    providerSlug?: string;
    providerTag?: string;
}): Promise<OpenRouterUsageReservation> {
    ...
    const usageRef = db.collection('openrouter_usage').doc(documentId);
    const now = new Date();
    const dailyCostDay = toKoreaDateKey(now);
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const existingState = usageSnapshot.data()?.state;
        ...
        if (!canReserveOpenRouterDailyCost({
            ...
            throw new OpenRouterDailyCostLimitError({
                ...
                estimatedCostUsd,
                dailyCostLimitUsd,
                retryAtMs: nextKoreaDayStartMs(now.getTime()),
            });
        }
        ...
        const providerSlug = params.providerSlug?.trim() || context.providerSlug?.trim();
        const providerTag = params.providerTag?.trim() || context.providerTag?.trim();
        transaction.set(usageRef, {
            ...
            logicalOperationHash: logicalOperationId,
            logicalOperationKeyVersion: OPENROUTER_LOGICAL_OPERATION_KEY_VERSION,
            requestId,
            estimatedCostUsd,
            ...
            dailyCostInflightAccounted: true,
            dailyCostLimitUsd,
            ...(typeof context.projectId === 'string' && SAFE_CONTEXT_ID.test(context.projectId)
                ? { projectId: context.projectId }
                : {}),
            ...(typeof context.batchId === 'string' && SAFE_CONTEXT_ID.test(context.batchId)
                ? { batchId: context.batchId }
                : {}),
            ...(Number.isInteger(context.frameIndex) && (context.frameIndex as number) >= 0
                ? { frameIndex: context.frameIndex }
                : {}),
            ...(Number.isInteger(context.seed) && (context.seed as number) >= 0
                ? { seed: context.seed }
                : {}),
            ...(typeof context.stage === 'string' && context.stage.trim()
                ? { stage: context.stage.trim().slice(0, 80) }
                : {}),
            ...(providerSlug ? { endpointProviderSlug: providerSlug.slice(0, 80) } : {}),
            ...(providerTag ? { endpointProviderTag: providerTag.slice(0, 80) } : {}),
 ...
 */
export async function settleOpenRouterUsageReservation(
    reservation: OpenRouterUsageReservation,
    entry: Omit<OpenRouterUsageEntry, 'operation'>,
): Promise<number> {
    if (!admin.apps.length) admin.initializeApp();
    const usageRef = db.collection('openrouter_usage').doc(reservation.documentId);
    const jobRef = db.doc(`users/${reservation.userId}/emoticonJobs/${reservation.jobId}`);
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const previousAccountedCostUsd = state === 'settled'
            ? toNonNegativeNumber(usage.costUsd as number | undefined)
            : 0;
        const suppliedCostUsd = toOptionalCost(entry.costUsd);
        accountedCostUsd = suppliedCostUsd ?? (state === 'settled'
        ...
        const reservedCostUsd = state === 'inflight'
            ? toNonNegativeNumber(usage.reservedCostUsd as number | undefined)
            : 0;
        ...
        transaction.update(usageRef, {
            state: 'settled',
            model: entry.model.trim() || reservation.model,
            ...
            costUsd: accountedCostUsd,
            costSource: suppliedCostUsd === null ? 'reservation_estimate' : 'provider_usage',
            providerRequestId: entry.requestId || reservation.requestId,
            ...(entry.providerSlug?.trim()
                ? { endpointProviderSlug: entry.providerSlug.trim().slice(0, 80) }
                : {}),
            ...(entry.providerTag?.trim()
        ...
        transaction.set(dailyCostRef, {
            ...
            ...(state === 'inflight'
                ...
                : { costAdjustmentCount: admin.firestore.FieldValue.increment(1) }),
            createdAt: dailyCostSnapshot.exists
                ? storedDailyCost.createdAt || admin.firestore.FieldValue.serverTimestamp()
                : admin.firestore.FieldValue.serverTimestamp(),
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        ...
        transaction.update(jobRef, {
            ...
            ...(Math.abs(actualDelta) > USD_EPSILON
                ...
                : {}),
            openRouterLastModel: entry.model.trim() || reservation.model,
            ...(entry.providerTag?.trim() || entry.providerSlug?.trim()
                ? { openRouterLastProvider: entry.providerTag?.trim() || entry.providerSlug?.trim() }
                : {}),
            openRouterUsageUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
    });
 ...
 */
export async function failOpenRouterUsageReservation(
    reservation: OpenRouterUsageReservation,
    params: { ambiguous: boolean; reason: string },
): Promise<void> {
    if (!admin.apps.length) admin.initializeApp();
    const usageRef = db.collection('openrouter_usage').doc(reservation.documentId);
    const jobRef = db.doc(`users/${reservation.userId}/emoticonJobs/${reservation.jobId}`);
    await db.runTransaction(async (transaction) => {
        ...
        if (!usageSnapshot.exists || usageSnapshot.data()?.state !== 'inflight') return;
        const usage = usageSnapshot.data() || {};
        const reservedCostUsd = toNonNegativeNumber(usage.reservedCostUsd as number | undefined);
        const dailyInflightWasAccounted = usage.dailyCostAccountingVersion
            === OPENROUTER_DAILY_COST_AGGREGATE_VERSION
            && usage.dailyCostInflightAccounted === true;
        const dailyCostIdentity = resolveOpenRouterDailyCostAggregateIdentity(usage);
        ...
        transaction.update(usageRef, {
            state: params.ambiguous ? 'uncertain' : 'failed',
            failureReason: params.reason.slice(0, 160),
            ...(params.ambiguous ? {} : { reservedCostUsd: 0 }),
            dailyCostAccountingVersion: OPENROUTER_DAILY_COST_AGGREGATE_VERSION,
            dailyCostInflightAccounted: params.ambiguous,
    ...
    try {
        ...
        const usage = {
            ...
            ...(typeof context.stage === 'string' && context.stage.trim()
                ? { stage: context.stage.trim().slice(0, 80) }
                : {}),
            ...(entry.providerSlug?.trim() || context.providerSlug?.trim()
                ? { endpointProviderSlug: (entry.providerSlug || context.providerSlug)!.trim().slice(0, 80) }
                : {}),
            ...(entry.providerTag?.trim() || context.providerTag?.trim()
        ...
        await db.runTransaction(async (transaction) => {
            const jobRef = userId && jobId
                ? db.doc(`users/${userId}/emoticonJobs/${jobId}`)
                : null;
            const reads = [transaction.get(document)];
            if (jobRef) reads.push(transaction.get(jobRef));
            ...
            if (jobRef && jobSnapshot?.exists) {
                transaction.update(jobRef, {
                    ...
                    ...(actualCostUsd !== null
                        ...
                        : {}),
                    openRouterLastModel: usage.model,
                    ...(entry.providerTag || entry.providerSlug || context.providerTag || context.providerSlug
                        ? {
