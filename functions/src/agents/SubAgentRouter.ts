 ...
 * Sub-Agent Types
 */
export type SubAgentType = 'code' | 'browser' | 'data' | 'file' | 'research' | 'general';

/**
 * Sub-Agent Route Decision
 */
 ...
 */
export class SubAgentRouter extends BaseAgent {
    private llm = LLMAdapterFactory.fromEnv();

    /**
     * Route patterns for quick matching (no LLM call needed)
     */
    private static readonly QUICK_ROUTES: Record<string, { type: SubAgentType; keywords: string[] }> = {
        code: {
            type: 'code',
     ...
     */
    private tryQuickRoute(userInput: string): RouteDecision | null {
        ...
        let bestMatch: { type: SubAgentType; score: number } | null = null;
     ...
     */
    private async classifyWithLLM(userInput: string): Promise<RouteDecision> {
