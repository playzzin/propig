...
const fetchHtmlWithTimeout = async (url: string, timeoutMs: number): Promise<string> => {
...
const findMetaContent = (html: string, attrName: 'name' | 'property', attrValue: string): string => {
...
const extractBasicMetadataFromHtml = (html: string) => {
  ...
  return {
    title: (ogTitle || titleMatch?.[1] || '').trim(),
...
const extractTextSnippetFromHtml = (html: string, maxLength: number): string => {
...
const deriveTagsFromTitle = (title: string, maxCount: number): string[] => {
...
const ensureNonEmptyTags = (tags: string[], url: string): string[] => {
...
const resolveMaybeRelativeUrl = (baseUrl: string, maybeRelative: string): string => {
  try {
...
export const extractBookmarkMetadata = https.onCall({}, async (request) => {
  ...
  try {
    logger.info('extractBookmarkMetadata:start', { url: safeUrl, uid: request.auth.uid });
    ...
    try {
      ...
      logger.warn('Failed to fetch HTML for bookmark analysis. Falling back to URL-only prompt.', {
        url: safeUrl,
    ...
    logger.info('extractBookmarkMetadata:context', {
      url: safeUrl,
  ...
  "favicon": "https://..."
    ...
    try {
      ...
      if (availableCategories.length > 0 && !availableCategories.includes(finalCategory)) {
        ...
        if (availableCategories.includes('참고')) finalCategory = '참고';
        else if (availableCategories.includes('기타')) finalCategory = '기타';
      ...
      logger.info('extractBookmarkMetadata:finalized', {
        url: safeUrl,
        titleLength: finalized.title?.length ?? 0,
        descriptionLength: finalized.description?.length ?? 0,
        tagsCount: finalized.tags?.length ?? 0,
...
export const extractBatchMetadata = https.onCall({}, async (request) => {
  ...
  try {
    ...
    const results: Array<{ url: string; metadata: unknown }> = [];
...
응답 형식:
{
  ...
  "favicon": "https://example.com/favicon.ico"
        ...
        logger.error(`Failed to extract metadata for ${url}:`, error);
      }
    }
