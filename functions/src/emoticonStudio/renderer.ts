...
import type { EmoticonExportFormat, EmoticonPlan } from './schema';
 ...
 */
async function removeDistantResidualComponents(pngBuffer: Buffer): Promise<Buffer> {
    const { data, info } = await sharp(pngBuffer)
        .ensureAlpha()
        .raw()
        .toBuffer({ resolveWithObject: true });
    ...
    const mainIndex = components.reduce((bestIndex, component, index) => (
        component.count > components[bestIndex].count ? index : bestIndex
    ), 0);
    ...
    components.forEach((component, index) => {
        if (index === mainIndex || component.count >= minArtifactPixels) return;
...
export async function normalizeGeneratedEmoticonPose(buffer: Buffer): Promise<Buffer> {
...
function bubbleOpacity(plan: EmoticonPlan, frameIndex: number): number {
...
function getBubbleLayout(plan: EmoticonPlan, position = plan.bubble.position): BubbleLayout | null {
    if (plan.bubble.style === 'none' || !plan.bubble.text.trim()) return null;
...
function createBubbleSvg(
    ...
    const text = lines.map((line, index) => (
        `<text x="${centerX}" y="${textY + index * 27}" text-anchor="middle" `
        + `font-family="Noto Sans KR, Noto Sans CJK KR, Arial, sans-serif" font-size="${fontSize}" `
        + `font-weight="800" fill="#20242D">${escapeXml(line)}</text>`
    )).join('');
...
async function getVisibleBounds(buffer: Buffer): Promise<FrameBounds> {
    const { data, info } = await sharp(buffer)
        .ensureAlpha()
        .raw()
        .toBuffer({ resolveWithObject: true });
...
function translateBounds(bounds: FrameBounds, left: number, top: number): FrameBounds {
    return {
...
function getBubbleBounds(layout: BubbleLayout, plan: EmoticonPlan): FrameBounds {
    ...
    if (layout.position === 'top') bounds.maxY += tailSize;
    if (layout.position === 'bottom') bounds.minY -= tailSize;
    if (layout.position === 'left') bounds.maxX += tailSize;
    if (layout.position === 'right') bounds.minX -= tailSize;
    return bounds;
}

function overlapArea(first: FrameBounds, second: FrameBounds): number {
...
async function resolveBubblePlacement(params: {
    ...
    height: number;
    transform: MotionTransform;
}): Promise<{ layout: BubbleLayout | null; left: number; top: number }> {
    ...
    const positions = [...new Set<BubblePosition>([
    ...
    let best: { layout: BubbleLayout; left: number; top: number; score: number } | null = null;
    for (const position of positions) {
        ...
        const score = overlapArea(
            translateBounds(characterBounds, placement.left, placement.top),
            getBubbleBounds(layout, params.plan),
        );
        if (!best || score < best.score) {
            best = { layout, ...placement, score };
        }
    }
    return best
        ? { layout: best.layout, left: best.left, top: best.top }
        : { layout: requestedLayout, ...requestedPlacement };
}
...
async function composeFrame(params: {
    ...
    if (bubble) composites.push({ input: bubble, left: 0, top: 0 });
    return sharp({
        create: {
            width: CANVAS_SIZE,
            height: CANVAS_SIZE,
...
async function encodeMp4(dir: string, fps: number): Promise<Buffer> {
...
async function encodePngZip(frames: Buffer[], plan: EmoticonPlan): Promise<Buffer> {
    const zip = new JSZip();
