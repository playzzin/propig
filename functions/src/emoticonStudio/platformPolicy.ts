...
const MANAGED_PLATFORMS = new Set<EmoticonOutputProfile['platform']>([
    'kakao',
    'line',
    'custom',
]);

export type ResolvedEmoticonGenerationPolicy = {
    outputProfile: EmoticonOutputProfile | undefined;
...
function storedPolicyOutputProfile(
    value: unknown,
    requested: EmoticonOutputProfile,
): { outputProfile: EmoticonOutputProfile; preferredFormat: EmoticonExportFormat; revision: number } | null {
    if (!isRecord(value) || value.enabled !== true || value.schemaVersion !== 1) return null;
    if (value.platform !== requested.platform || value.type !== requested.type) return null;
    if (value.id !== `${requested.platform}-${requested.type}`) return null;
    if (!Number.isInteger(value.revision) || Number(value.revision) < 0) return null;
    if (!isRecord(value.profile)) return null;
    const profile = value.profile;
    if (profile.platform !== requested.platform || profile.type !== requested.type) return null;
    if (profile.transparentBackground !== true || profile.submissionCandidate !== false) return null;
    if (!Array.isArray(profile.allowedFormats)) return null;
    ...
    const maxDuration = isRecord(limits.maxDurationMs)
        ...
        : undefined;
    const candidate = emoticonOutputProfileSchema.safeParse({
        ...
        ...(maxDuration !== undefined ? { maxDurationMs: maxDuration } : {}),
        ...(Number.isInteger(profile.defaultLoopCount)
            ? { loopCount: profile.defaultLoopCount }
            : {}),
        ...(typeof profile.sourceUrl === 'string' ? { sourceUrl: profile.sourceUrl } : {}),
        ...(typeof profile.checkedAt === 'string' ? { checkedAt: profile.checkedAt } : {}),
        submissionCandidate: false,
    });
    if (!candidate.success) return null;
    return {
        outputProfile: candidate.data,
        preferredFormat: profile.preferredFormat as EmoticonExportFormat,
 ...
 */
export async function resolveEmoticonGenerationPolicy(params: {
    outputProfile?: EmoticonOutputProfile;
    formats: EmoticonExportFormat[];
}): Promise<ResolvedEmoticonGenerationPolicy> {
    const fallback: ResolvedEmoticonGenerationPolicy = {
        outputProfile: params.outputProfile,
        formats: Array.from(new Set(params.formats)),
        source: 'fallback',
    };
    const requested = params.outputProfile;
    if (!requested || !MANAGED_PLATFORMS.has(requested.platform)) return fallback;

    const policyId = `${requested.platform}-${requested.type}`;
    try {
        const snapshot = await db.collection(POLICY_COLLECTION).doc(policyId).get();
