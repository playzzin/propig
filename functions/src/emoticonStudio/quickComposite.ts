...
import { normalizeEmoticonPlanForOutputProfile } from './outputPlanPolicy';

export const EFFICIENT_LOCAL_COMPOSITE_VERSION = 'efficient-local-composite-v1';

type MotionType = EmoticonPlan['action']['motionType'];

function cleanInstruction(instruction: string): string {
    const withoutProfile = instruction.split('[확정 캐릭터 DNA]')[0]?.trim() || instruction.trim();
    return withoutProfile.replace(/\s+/g, ' ').slice(0, 320);
}

function inferMotionType(instruction: string): MotionType {
    if (/(점프|뛰어오|폴짝|도약|jump|bounce)/i.test(instruction)) return 'jump';
    if (/(손.*흔들|인사|안녕|반가|wave|greet)/i.test(instruction)) return 'wave';
    if (/(달리|뛰어|걷|춤|회전|날아|run|walk|dance|spin|fly)/i.test(instruction)) return 'dynamic';
    if (/(말하|외치|소리|입.*벌|대사|말풍선|talk|shout|say)/i.test(instruction)) return 'talk';
    if (/(흔들|부들|화나|분노|아니|고개.*젓|shake|angry)/i.test(instruction)) return 'shake';
    return 'bob';
}

function defaultBubble(): EmoticonBubble {
    return {
        text: '',
        style: 'none',
        position: 'top',
...
function normalizeAnimatedBubble(bubble: EmoticonBubble, frameCount: number): EmoticonBubble {
    ...
    const cues = [...bubble.timeline.cues]
        ...
        .flatMap((cue) => {
            ...
            if (startFrame > lastFrame) return [];
    ...
    return {
        ...
        timeline: {
            ...bubble.timeline,
            mode,
            startFrame: Math.min(lastFrame, bubble.timeline.startFrame),
            endFrame: bubble.timeline.endFrame === null
                ? null
                : Math.min(lastFrame, bubble.timeline.endFrame),
            cues,
        },
    };
}
 ...
 */
export function buildEfficientCompositePlan(params: {
    ...
    const bubble = normalizeAnimatedBubble(
        params.request.bubbleOverride || defaultBubble(),
        motion.frameCount,
    );
    const base = emoticonPlanSchema.parse({
        ...
        action: {
            ...
            emotion: bubble.text || bubble.timeline.cues[0]?.text ? '대사 표현' : '원본 표정 유지',
    ...
    const normalized = normalizeEmoticonPlanForOutputProfile(base, params.outputProfile);
    return emoticonManualStoredPlanSchema.parse(normalized);
}
