...
import {
    ...
    emoticonCharacterSheetPlanSchema,
    ...
    type EmoticonCharacterSheetPlan,
...
import { resolveEmoticonStudioImageProviderName } from './imageGenerationProvider';
...
function buildCharacterSheetPlanJsonSchema(itemCount: number) {
    return {
        type: 'object',
        additionalProperties: false,
        properties: {
...
function truncateModelText(value: unknown, maxLength: number): unknown {
 ...
 */
export function normalizeEmoticonFrameSequenceCandidate(value: unknown): unknown {
    ...
    const frames = Array.isArray(candidate.frames)
        ? candidate.frames.map((value) => {
            ...
            return {
                ...frame,
                posePrompt: truncateModelText(frame.posePrompt, 700),
                expressionPrompt: truncateModelText(frame.expressionPrompt, 400),
                continuityPrompt: truncateModelText(frame.continuityPrompt, 400),
            };
        })
        : candidate.frames;
    if (Array.isArray(frames) && frames.length >= 2) {
        ...
        if (
            ...
            const firstFrame = first as Record<string, unknown>;
            const lastFrame = last as Record<string, unknown>;
            ...
            const normalizedLastPose = typeof lastFrame.posePrompt === 'string'
                ...
                : '';
            const duplicateLoopPose = Boolean(
                normalizedFirstPose
                && normalizedLastPose
            ...
            frames[lastIndex] = {
                ...
                posePrompt: truncateModelText(
                    duplicateLoopPose
                        ...
                        : `${String(lastFrame.posePrompt || '')} Final-loop contract: remain visibly different from frame 0 with at least one hand or foot still partway through recovery.`,
                    700,
                ),
                expressionPrompt: truncateModelText(
                    `${String(lastFrame.expressionPrompt || '')} Keep one visible eye, eyebrow, cheek, or mouth detail between the preceding peak and frame 0; do not duplicate frame 0 exactly.`,
                    400,
                ),
                continuityPrompt: truncateModelText(
                    `${String(lastFrame.continuityPrompt || '')} The encoder jumps directly from this frame to frame 0, so this must be a distinct in-between recovery rather than a duplicate hold.`,
                    400,
                ),
            };
        }
    ...
    return {
        ...candidate,
        sequenceSummary: truncateModelText(candidate.sequenceSummary, 320),
        frames,
    };
}

function normalizeFrameSemanticText(value: string): string {
 ...
 */
export function validateEmoticonFrameSequenceSemantics(params: {
    plan: EmoticonPlan;
    sequence: EmoticonFrameSequence;
}): string[] {
    ...
    const combinedSignatures = frames.map((frame, index) => (
        `${poseSignatures[index]}|${expressionSignatures[index]}|${frame.phase}`
    ));
    ...
    if (!['start', 'anticipation'].includes(frames[0]?.phase || '')) {
        issues.push('frame 0 must begin with start or anticipation');
    }
    if (frames[frames.length - 1]?.phase !== 'loop-return') {
        issues.push('the final frame must be a distinct loop-return');
    }
    const phases = new Set(frames.map((frame) => frame.phase));
    if (phases.size < 4) issues.push('the timeline needs at least four distinct animation phases');
...
const cachedPresetsSchema = emoticonPresetSchema.array().max(64);
...
function parseMockMessageJson(value: unknown): Record<string, unknown> | null {
...
function collectMockMessagePayloads(
    ...
    for (const message of messages) {
        ...
        for (const part of message.content) {
            if (!isMockRecord(part) || part.type !== 'text') continue;
...
function collectMockSystemText(messages: Array<Record<string, unknown>>): string {
    return messages
...
function countMockImageParts(messages: Array<Record<string, unknown>>): number {
    return messages.reduce((total, message) => {
        if (!Array.isArray(message.content)) return total;
        return total + message.content.filter(
            (part) => isMockRecord(part) && part.type === 'image_url',
        ).length;
    }, 0);
}

function mockPayloadValue(
...
function boundedMockText(value: unknown, fallback: string, maximumLength: number): string {
...
function createMockActionPlan(params: {
    ...
    const requestedPreference = params.systemText.match(
        /User motion preference:\s*(auto|stable|dynamic)/i,
    )?.[1]?.toLowerCase();
    ...
    return {
        ...
        motionType: stable ? 'bob' as const : 'dynamic' as const,
        renderMode: stable ? 'stable' as const : 'dynamic' as const,
        durationMs: 1000,
        frameCount: 12,
        fps: 12,
...
function schemaArrayLength(
    ...
    const property = properties && isMockRecord(properties[propertyName])
        ...
        : null;
    const exactLength = property?.minItems === property?.maxItems
        ? Number(property?.minItems)
        : Number.NaN;
    return Number.isInteger(exactLength) && exactLength > 0 ? exactLength : null;
}

function createMockFrameSequence(frameCount: number) {
    const phaseFor = (index: number) => {
        if (index === 0) return 'start' as const;
        if (frameCount === 4) {
            return index === 1 ? 'action' as const
                : index === 2 ? 'opposite' as const
                    : 'loop-return' as const;
        }
        if (frameCount === 5) {
            const shortCycle = [
                'start',
        ...
        if (index === 1) return 'anticipation' as const;
        if (index === frameCount - 1) return 'loop-return' as const;
        if (index === frameCount - 2) return 'recovery' as const;
        const cycle = ['action', 'opposite', 'follow-through'] as const;
...
function createMockStructuredValue(params: {
    ...
    const instruction = boundedMockText(
        mockPayloadValue(payloads, 'originalUserInstruction'),
        '캐릭터가 밝게 인사하는 반복 동작을 만듭니다.',
        800,
    );
    ...
    switch (params.schemaName) {
        case 'emoticon_direction': {
            const requestedPresetCount = Number(
                systemText.match(/suggestedPresets must contain exactly\s+(\d+)/i)?.[1] || 0,
            );
            const presetCount = Number.isInteger(requestedPresetCount)
                ? Math.max(0, Math.min(64, requestedPresetCount))
                : 0;
            return {
                directorSummary: boundedMockText(
                    `목업 감독 계획: ${instruction}`,
                    '목업 감독 계획',
                    320,
                ),
                characterProfile: {
        ...
        case 'emoticon_character_sheet_plan_v1': {
            ...
            const request = boundedMockText(
                mockPayloadValue(payloads, 'request'),
                '캐릭터 참고 시트',
                320,
            );
            return {
        ...
        case 'emoticon_frame_sequence':
            return createMockFrameSequence(
                schemaArrayLength(params.schema, 'frames') || 12,
            );
...
async function requestStructuredChat(params: {
    ...
    if (resolveEmoticonStudioImageProviderName() === 'mock') {
    ...
    try {
        ...
        if (controller.signal.aborted) {
            throw new Error(
                'OpenRouter structured analysis timed out; its cost state is uncertain and automatic replay was blocked.',
            );
        }
        throw error;
    }
    ...
    if (!response.ok) {
        ...
        const error = new Error(
            payload.error?.message || raw.slice(0, 1000) || `OpenRouter HTTP ${response.status}`,
        );
...
export function normalizeEmoticonPlanTiming(
    plan: EmoticonPlan,
    ...
    const requestedFrameCount = Math.max(
        safeMinimumFrameCount,
        Math.min(24, Math.round(plan.action.frameCount)),
    );
    ...
    for (let fpsCandidate = 2; fpsCandidate <= 18; fpsCandidate += 1) {
        for (let frameCandidate = safeMinimumFrameCount; frameCandidate <= 24; frameCandidate += 1) {
            ...
            if (durationCandidate < 700 || durationCandidate > 3000) continue;
 ...
 * durations.
 */
export function applyEmoticonMotionOverride(
    plan: EmoticonPlan,
    override?: EmoticonMotionOverride,
): EmoticonPlan {
...
export function applyEmoticonMotionPolicy(params: {
    ...
    if (!shouldUseDynamicFrames) return plan;
    return {
        ...plan,
        action: {
            ...plan.action,
...
export async function analyzeEmoticonDirection(params: {
    ...
    const payload = await requestStructuredChat({
        ...
        maxTokens: useCachedAnalysis
            ? 2200
            : Math.min(16_000, 4_200 + ((params.templateItemCount || 0) * 180)),
    ...
    return normalizeEmoticonPlanTiming(
 ...
 */
export async function analyzeEmoticonCharacter(params: {
    apiKey: string;
    model: string;
    fallbackModels: string[];
    sourceImageUrl: string;
            ...
            },
            {
                ...
                content: [
                    ...
                    ...referenceUrls.map((url) => ({
                        type: 'image_url' as const,
                        image_url: { url, detail: resourceMode === 'efficient' ? 'low' as const : 'high' as const },
                    })),
                ],
            },
        ],
    ...
    let parsed = emoticonCharacterAnalysisSchema.safeParse(
        extractJsonObject(payload.choices?.[0]?.message?.content || ''),
    );
    ...
    if (!parsed.success) {
        ...
        parsed = emoticonCharacterAnalysisSchema.safeParse(
            extractJsonObject(payload.choices?.[0]?.message?.content || ''),
        );
    }
    if (!parsed.success) {
        throw new Error(
            `OpenRouter returned an invalid character profile: `
            + formatFirstSchemaIssue(parsed.error),
        );
    }
...
export function characterAnalysisToLegacyProfile(
    analysis: EmoticonCharacterAnalysis,
): EmoticonCharacterProfile {
    return emoticonCharacterProfileSchema.parse({
...
export async function planEmoticonCharacterSheet(params: {
    apiKey: string;
    model: string;
    fallbackModels: string[];
    ...
    itemCount: number;
    reasoningEffort?: 'low';
}): Promise<EmoticonCharacterSheetPlan> {
    ...
    const payload = await requestStructuredChat({
        ...
        maxTokens: Math.min(12_000, 1000 + itemCount * 190),
        reasoningEffort: params.reasoningEffort,
    });
    const parsed = emoticonCharacterSheetPlanSchema.safeParse(
        extractJsonObject(payload.choices?.[0]?.message?.content || ''),
    );
    if (!parsed.success) {
        throw new Error(
            `OpenRouter returned an invalid character sheet plan: `
            + formatFirstSchemaIssue(parsed.error),
        );
    }
...
export async function planEmoticonFrameSequence(params: {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        let parsed: ReturnType<typeof emoticonFrameSequenceSchema.safeParse>;
        try {
            const payload = await requestStructuredChat({
                apiKey: params.apiKey,
                model: params.model,
            ...
            const content = payload.choices?.[0]?.message?.content || '';
            parsed = emoticonFrameSequenceSchema.safeParse(
                normalizeEmoticonFrameSequenceCandidate(extractJsonObject(content)),
            );
        } catch (error) {
        ...
        if (!semanticIssues.length) return parsed.data;
