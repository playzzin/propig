...
const CHARACTER_COLORS = ['#FF7A59', '#7C5CFC', '#3B82F6', '#F59E0B', '#EC4899'] as const;

function stableRequestIdentity(request: ImageGenerationProviderRequest): string {
    return JSON.stringify({
        ...
        resourceMode: request.resourceMode || null,
        ...(request.kind === 'sprite-sheet'
            ? { layout: request.layout, directions: request.directions }
            : {}),
    });
}

function digestRequest(request: ImageGenerationProviderRequest): Buffer {
    return createHash('sha256').update(stableRequestIdentity(request)).digest();
}
...
function characterMarkup(params: {
    ...
    const line = (start: { x: number; y: number }, end: { x: number; y: number }, inner: string) => [
...
async function renderPose(request: ImageGenerationProviderRequest, digest: Buffer): Promise<Buffer> {
    const phase = ((digest[0] * 256 + digest[1]) / 65_536) * Math.PI * 2;
...
async function renderSpriteSheet(
    request: Extract<ImageGenerationProviderRequest, { kind: 'sprite-sheet' }>,
    digest: Buffer,
): Promise<Buffer> {
