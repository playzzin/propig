 ...
 */
export function storagePathFromVideoStudioUrl(value: unknown, prefix: string): string | null {
    if (typeof value !== 'string' || !value.trim() || !prefix) return null;
    ...
    try {
        ...
        const legacyStoragePath = url.hostname === 'storage.googleapis.com'
            ...
            : url.hostname.endsWith('.storage.googleapis.com')
                ? decodedPathname
                : null;
        return legacyStoragePath?.startsWith(prefix) ? legacyStoragePath : null;
    } catch {
        return null;
    }
}
