...
export default function StoryboardSceneProductionEditor({
  ...
  return (
    <SceneProductionRow $status={scene.video.status}>
      <SceneMedia>
      ...
      <SceneBrief>
        ...
        {hasPendingVideoChanges ? (
          <MotionChangeNotice role="status">
            ...
            <span>
              <strong>수정 내용이 아직 영상에 반영되지 않았습니다.</strong> 기존
              영상은 새 제작을 시작하기 전까지 그대로 유지됩니다. 아래 “수정
              내용으로 다시 제작”을 누르면 이 장면만 새 시안으로 만듭니다.
            </span>
          </MotionChangeNotice>
        ) : null}
        <SceneControls>
          ...
          <MotionControl aria-label={`${scene.order}번 장면 움직임 강도`}>
        ...
        <AudioModeSection>
          ...
          {sceneAudioMode === "dialogue" ? (
            <DialogueComposer $tone={dialogueTiming.tone}>
              ...
              />
              <small id={`video-dialogue-help-${scene.id}`}>
                입력한 문장을 그대로 말하도록 요청하며, 화면 자막은 생성하지
                않습니다.
              </small>
        ...
        <SceneAdvancedDetails>
          <summary>
            <span>
              <i className="fas fa-sliders" aria-hidden="true" /> 장면 고급 설정
            </span>
            <small>참조 사진 · 장면 연결 · 자르기</small>
              ...
              <ReferenceChoiceList>
                {referenceAssets.map((asset) => {
                  ...
                  return (
                    ...
                    >
                      <i
                        className={isSelected ? "fas fa-check" : "fas fa-image"}
                        aria-hidden="true"
                      />
          ...
          <ClipEditor>
            <summary>
              <span>
                <i className="fas fa-scissors" aria-hidden="true" /> 클립 다듬기
              </span>
            ...
            <ClipEditorGrid>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    onPatchVideo({ trimEndSeconds: Number(event.target.value) })
                  }
                  disabled={projectBusy || isSceneBusy}
                />
                <small>초</small>
