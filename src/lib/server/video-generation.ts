...
// The video catalog changes independently from app releases. Keep it fresh enough
// for automatic selection, while sharing an in-flight fetch across simultaneous
// storyboard estimates so one edit cannot fan out into many provider requests.
...
const VideoReferenceImageSchema = z
  .string()
  .trim()
  .min(1)
  .max(280_000)
...
const OpenRouterVideoExecutionInputsSchema = z.object({
  ...
  lastFrame: z.boolean(),
  visualReferences: z.boolean(),
  audioMode: z.enum(STORYBOARD_VIDEO_AUDIO_MODES),
});
...
export function deriveVideoInfraHint(rawMessage: string): VideoInfraHint {
  ...
  if (
    ...
    return {
      ...
      message:
        "참조 사진에 실제 인물이 포함되었거나 그렇게 감지되어 모델이 요청을 받지 않았습니다. 이 장면에서 사진 없이 다시 만들거나 인물 사진을 교체해 주세요.",
    };
  }
  if (
  ...
  if (
    ...
    return {
      ...
      message: "OpenRouter API 인증 또는 권한을 확인하세요.",
    };
  }
  if (lower.includes("rate limit") || lower.includes("429")) {
    return {
      reasonCode: "rate_limited",
      message: "OpenRouter 요청 한도에 도달했습니다. 잠시 후 다시 시도하세요.",
    };
  }
  if (
    ...
    return {
      ...
      message: "OpenRouter 영상 생성 시간이 초과되었습니다.",
    };
  }
  if (
  ...
  if (
    ...
    return {
      ...
      message: "OpenRouter가 영상 생성 요청을 거부했습니다.",
    };
  }
  return {
    reasonCode: "unknown",
    message: "OpenRouter 영상 생성에 실패했습니다. 모델과 요청을 확인하세요.",
  };
}
...
const parseVideoResponse = async (
  response: Response,
...
export async function preflightOpenRouterVideo(params: {
  ...
  const selectionReasons = [
    `OpenRouter ${selected.catalogSource === "live" ? "실시간" : "최근"} 카탈로그 ${selected.catalogModelCount}개 모델에서 조건을 다시 확인했습니다.`,
    `${selected.compatibleModelCount}개 기술 호환 모델 중 ${params.qualityMode === "final" ? "완성도와 지원 기능" : "요청 해상도와 시안 속도"}을 우선해 자동 선택했습니다.`,
  ];
  if (requestedInputs.firstFrame)
    selectionReasons.push(
      "시작 프레임 연속성을 지원하는 모델만 대상으로 했습니다.",
    );
  if (requestedInputs.lastFrame)
    selectionReasons.push("도착 프레임을 지원하는 모델만 대상으로 했습니다.");
  if (visualReferencesCanBeApplied)
    selectionReasons.push(
      "일관성 참조 이미지를 지원하는 모델만 대상으로 했습니다.",
    );
  if (requestedInputs.audioMode === "dialogue")
    selectionReasons.push(
      "대사·립싱크 기능이 확인된 모델만 대상으로 했습니다.",
    );
  ...
  const credit = creditResult.ok
    ...
    : {
        ...
        isSufficient: null,
        message:
          "OpenRouter 잔액을 지금 확인하지 못했습니다. 생성 시 제공자에서 최종 확인합니다.",
        fetchedAt: null,
        refreshAfter: null,
      };
  if (credit.isSufficient === false) {
    warnings.push(
      "OpenRouter 잔액이 이 장면의 예상 비용보다 부족합니다. 충전 후 다시 시도하세요.",
    );
  }
  ...
  const policy = !hasVisualInput
    ? {
        ...
        message:
          "텍스트 기반 제작입니다. 참조 사진에 대한 제공사 심사는 적용되지 않습니다.",
        recommendedActions: [],
      }
    : params.knownInputImagePrivacyBlock
      ? {
          ...
          message:
            "같은 장면의 참조 사진이 제공사 정책에서 이미 제한되었습니다. 기술 지원 여부와 사진 허용 여부는 별개입니다.",
          recommendedActions: [
            "사진 없이 새 테이크 만들기",
            "권리와 초상 사용 동의가 확인된 대체 레퍼런스로 교체하기",
          ],
        }
      : {
          ...
          automaticRetryAllowed: true,
          message:
            "선택 모델은 프레임·참조 입력을 기술적으로 지원합니다. 실제 인물 사진의 허용 여부는 생성 요청 시 제공사 정책으로 최종 판단됩니다.",
          recommendedActions: [
            "실사 인물 사진은 권리와 초상 사용 동의를 확인하기",
            "차단되면 모델 자동 교체 대신 사진 없이 새 테이크 또는 대체 레퍼런스를 사용하기",
          ],
        };
...
export async function generateOpenRouterVideo(
  ...
  if (canResume && resumeCheckpoint) {
    ...
    const [selected, executionCredit] = await Promise.all([
      ...
      getOpenRouterCredits(runtime.openRouterApiKey, true).catch(() => null),
    ]);
    if (
      executionCredit &&
      selected.estimatedCostUsd !== null &&
      ...
      throw new OpenRouterVideoRequestError({
        ...
        message:
          "OpenRouter 잔액이 실제 실행 비용보다 부족합니다. https://openrouter.ai/settings/credits 에서 충전한 뒤 다시 시도해 주세요.",
      });
    }
    if (audioMode === "dialogue") {
      const resolvedTiming = analyzeStoryboardDialogueTiming(
        payload.dialogue,
        selected.resolvedDuration,
      );
      if (
        !resolvedTiming.fitsSupportedDuration ||
        resolvedTiming.tone === "over"
      ) {
        throw new OpenRouterVideoRequestError({
          status: 422,
...
export async function downloadOpenRouterVideo(
  ...
  const headers = buildOpenRouterVideoDownloadHeaders(
    safeSourceUrl,
    runtime.openRouterApiKey,
  );
  const controller = new AbortController();
