...
export function resolveFfmpegBinaryPath(reportedPath: string | null): string;
...
export async function mergeVideos(params: {
    ...
    return withTempDir('video-merge-', async (dir) => {
        ...
        if (!params.backgroundMusicUrl) {
            return readFile(/* turbopackIgnore: true */ assembledPath);
        }
        ...
        return readFile(/* turbopackIgnore: true */ mixedPath);
    });
}
