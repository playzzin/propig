...
import { ragSystem, vectorStore } from '../rag/VectorStore';
import { toolRegistry } from '../tools/ToolRegistry';
import { metricsCollector } from '../monitoring/MetricsCollector';
import { workflowEngine, createCodeReviewWorkflow } from '../workflow/WorkflowEngine';
import { multimodalProcessor } from '../multimodal/MultimodalProcessor';
import { retryWithBackoff } from '../resilience/ErrorHandler';
import { OrchestratorAgent } from '../Orchestrator';

/**
 * Example 1: 기본 대화 흐름 (메모리 + 에이전트)
 */
export async function basicConversationExample() {
    console.log('\n=== Example 1: Basic Conversation ===\n');

    // 1. 세션 생성
    const sessionId = await memoryManager.createSession('user123', {
        platform: 'web',
        timestamp: Date.now(),
    });
    ...
    // 2. 첫 번째 메시지
    await memoryManager.addMessage(sessionId, 'user', 'Hello! My name is John.');
    ...
    // 5. 두 번째 메시지 (컨텍스트 유지)
    await memoryManager.addMessage(sessionId, 'user', 'What was my name again?');
...
export async function ragExample() {
    ...
    const documents = [
        ...
        {
            content:
                'Firebase is a Backend-as-a-Service platform that provides authentication, database, and hosting.',
    ...
    const ragContext = await ragSystem.retrieveContext(query, 2);
    ...
    // 3. 소스와 함께 답변 생성
    const answerWithSources = await ragSystem.getAnswerWithSources(query);
...
export async function toolExecutionExample() {
    ...
    // 3. JSON 파싱 툴
    const jsonResult = await toolRegistry.executeTool('parse_json', {
...
export async function multimodalExample() {
    ...
    const fileMetadata = await multimodalProcessor.uploadFile(
        mockImageBuffer,
        'test-image.png',
        'image/png',
        'user123',
    ...
    // 세션 파일 조회
    const sessionFiles = await multimodalProcessor.getSessionFiles('session_abc');
...
export async function integratedScenario() {
    ...
    // 8. 최종 컨텍스트 조회
    const finalContext = await memoryManager.getConversationContext(sessionId);
