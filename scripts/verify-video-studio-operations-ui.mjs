...
assert.match(page, /useCurrentUserAccess\(\)/, 'Video Studio must resolve administrator access before rendering paid controls.');
assert.match(page, /if \(!currentUser\)[\s\S]*로그인한 관리자만 사용할 수 있습니다/);
assert.match(page, /if \(access\.role !== 'admin'\)/);
assert.match(page, /import\('\.\/components\/TestPanel'\)/, 'The paid test panel must stay out of the guest initial bundle.');
assert.match(page, /aria-label="영상 프로젝트 선택"/);
