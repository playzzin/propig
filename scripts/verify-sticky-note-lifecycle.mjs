...
globalThis.window = { localStorage: {getItem:k=>storage.get(k)??null,setItem:(k,v)=>{localWrites.push(k);storage.set(k,v)}}, setTimeout:fn=>{timers.set(++nextTimer,fn);return nextTimer;},clearTimeout:id=>timers.delete(id)};
...
await reset(); id=await add();
await act(async()=>api.updateNote(id,{content:'보존할 본문',memoType:'checklist',checklistItems:[{id:'line',text:'항목',isChecked:true,comments:[]}],priority:'high',reminderAt:100}));
await unmount();
const restoredRaw=storage.get('sticky_notes:v1:A'); localWrites.length=0;
await mount();
assert.equal(localWrites.filter(k=>k==='sticky_notes:v1:A').length,0);
assert.equal(storage.get('sticky_notes:v1:A'),restoredRaw);
assert.equal(api.notes[0].checklistItems[0].isChecked,true);
assert.equal(api.notes[0].reminderAt,100);
await act(async()=>{api.updateNote(id,{content:'복원 직후 편집'});renderer.unmount();await drain();});
assert.equal(saved()[0].content,'복원 직후 편집');
assert(localWrites.includes('sticky_notes:v1:A'));
console.log('PASS normal hydration does not rewrite durable payload; smart metadata preserved and immediate edit remains durable');
const normalSetItem=window.localStorage.setItem;
window.localStorage.setItem=(key,value)=>{if(key==='sticky_notes:v1:A')throw Error('quota');normalSetItem(key,value);};
await mount(); assert.equal(api.notes[0].content,'복원 직후 편집');assert.equal(api.storageError,null);
await unmount();window.localStorage.setItem=normalSetItem;
console.log('PASS healthy persisted notes hydrate even when notes storage quota rejects writes');
storage.set('sticky_notes:v1:A','{broken');
window.localStorage.setItem=(key,value)=>{if(key.startsWith('sticky_notes:v1:A'))throw Error('quota');normalSetItem(key,value);};
await mount();await act(async()=>api.createNote());
assert.equal(storage.get('sticky_notes:v1:A'),'{broken');assert.equal(api.notes.length,0);
assert(api.storageError.includes('원본'));await unmount();window.localStorage.setItem=normalSetItem;
console.log('PASS failed recovery backup preserves corrupt original and blocks subsequent local mutation');
console.log('12 lifecycle regressions PASS; real React hook + esbuild, mocked Firebase/browser storage, no remote access.');
