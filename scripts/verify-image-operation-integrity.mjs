...
for (const file of files) {
  ...
  function fixture() {
    ...
    const db = { collection, runTransaction: async callback => {
      const staged = [];
      const result = await callback({ get: async r => ({ data: () => docs.get(r.key) }), set: (r, data, opts) => staged.push([r.key, data, opts]) });
    ...
    const context = vm.createContext({ createHash, gzipSync, gunzipSync, Buffer, Date, process: { env: {} }, db, adminDb: db, admin, firebaseAdmin: admin, ApiError });
    ...
    const seed = (status, extra = {}) => {
      ...
      docs.set(budgetPath, { reservedUsd: 1, spentUsd: 0 });
    };
    ...
    return { ...api, docs, stats, payload, key, opPath, budgetPath, seed, reserve, conflict };
  }
  const cases = [
    ...
    ['failed retry', async f => { f.seed('failed', { budgetSettled: true }); f.docs.set(f.budgetPath, { reservedUsd: 0, spentUsd: 0 }); assert.equal((await f.reserve()).cached, null); assert.equal(f.docs.get(f.budgetPath).reservedUsd, 1); assert.equal(f.stats.writes, 2); }],
